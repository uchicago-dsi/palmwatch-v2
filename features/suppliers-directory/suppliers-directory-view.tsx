...
import styles from "./suppliers-directory.module.css";
...
function supplierTypeLabel(type: SupplierEntry["type"]): string {
  if (type === "owner") {
    return "Mill owner";
  }
  if (type === "group") {
...
function getRowMode(supplier: SupplierEntry, filter: FilterType): RowMode {
  if (supplier.type === "both") {
    if (filter === "all") {
      return {
        dual: true,
        ownerHref: supplier.ownerHref,
        groupHref: supplier.groupHref,
      };
    }
    return {
      dual: false,
      href: filter === "owner" ? supplier.ownerHref : supplier.groupHref,
      displayType: filter,
    };
  }
  return { dual: false, href: supplier.href, displayType: supplier.type };
}

function getSearchHref(supplier: SupplierEntry): string {
  if (supplier.type === "both") {
    return supplier.groupHref;
  }
  return supplier.href;
}
...
export function SuppliersClient({ suppliers, stats }: Props) {
  ...
  const searchResults = React.useMemo(
    () =>
      showResults
        ? suppliers.filter((c) => c.label.toLowerCase().includes(q)).slice(0, 8)
        : [],
    [suppliers, q, showResults]
  );
    ...
    <div className={styles.page}>
      ...
      <section className={styles.hero}>
        <h1 className={styles.heroTitle}>
          Suppliers in the palm oil supply chain
        </h1>
        <p className={styles.heroBody}>
          The mill owners and corporate groups that own and operate palm oil
          mills. Mill owners run mills directly; corporate groups are parent
          companies that control one or more mill owners.
        </p>
      </section>
      ...
      <div className={styles.searchWrap}>
        <div className={styles.searchBar}>
          ...
          <input
            aria-activedescendant={
              activeIdx >= 0 ? `supplier-result-${activeIdx}` : undefined
            }
        ...
        {showResults && (
          <div
            ...
            id="supplier-search-results"
            ref={resultsRef}
            role="listbox"
          >
            {searchResults.length > 0 ? (
              searchResults.map((supplier, i) => (
                <Link
                  aria-selected={i === activeIdx}
                  className={`${styles.searchResultItem} ${i === activeIdx ? styles.searchResultActive : ""}`}
                  href={getSearchHref(supplier)}
                  id={`supplier-result-${i}`}
                  key={supplier.label}
                  role="option"
                >
      ...
      <section>
        <div className={styles.tableHeader}>
          <div className={styles.tableHeaderLeft}>
            <span className={styles.sectionTitle}>All suppliers</span>
            <span className={styles.tableCount}>
              {sorted.length.toLocaleString()} suppliers
            </span>
          </div>
        ...
        <div className={styles.tableWrap}>
          <table className={styles.table}>
            <thead>
              <tr>
                <th
                  className={`${styles.thSupplier} ${sortKey === "label" ? styles.thActive : ""}`}
            ...
            <tbody>
              {pageRows.length > 0 ? (
                pageRows.map((supplier) => {
                  const mode = getRowMode(supplier, filter);
                  if (mode.dual) {
                    return (
                      <tr
