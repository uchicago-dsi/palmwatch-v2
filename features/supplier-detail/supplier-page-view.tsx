...
import { SupplierStatsGrid } from "./components/supplier-stats-grid";
import styles from "./supplier.module.css";

// ── Types ─────────────────────────────────────────────────────────────────────

export interface SupplierPageViewProps {
...
function CumulativeChart({ data }: { data: CumulativePoint[] }) {
  ...
  const gradientId = "supplierCumulativeGrad";
...
export function SupplierPageView({
  ...
  aboutContent,
  deforestationMap,
}: SupplierPageViewProps) {
  const { brandUsage, averageCurrentRisk, uniqueMills, uniqueCountries } =
    pageData;

  const displayName = useMemo(() => toDisplayCase(name), [name]);
    ...
    <div className={styles.page}>
      ...
      <div>
        <nav className={styles.breadcrumb}>
          <Link className={styles.breadcrumbLink} href="/suppliers">
            Suppliers
          </Link>
        ...
        <div className={styles.headerRow}>
          <h1 className={styles.supplierName}>{displayName}</h1>
          <span
      ...
      {normalizedBrandUsage.length > 0 && (
        <div className={styles.matrixCard}>
          <p className={styles.matrixTitle}>
            Consumer brands sourcing from this supplier
          </p>
